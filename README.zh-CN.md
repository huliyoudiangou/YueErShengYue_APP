...
- 点击开始播放后 **直接进入播放页**
- 暂停或关闭应用后再次打开，**悬浮窗可恢复**
- **倍速模型**：设置页 = 全局默认；播放页可选「全局」（跟随设置）或「本书」单独倍速

### 书库与发现
- **首页分区**：继续播放 · 最近添加 · 推荐图书 · 再次收听 · 我的收藏
- **书库浏览**：封面 + 书名网格（每行三本），无限向下滚动
- **书库筛选排序**：标题、演播、创建/修改/添加时间；再次点击切换升序/降序
- **全局搜索**：跨全部媒体库索引结果
- **详情页**：封面、演播、简介（可展开）、片头片尾设置
- 书名过长 **单行跑马灯**；演播展示如「演播：张三」
- **收藏**：详情页右上角爱心；首页「我的收藏」列表
- 长按 **继续播放** 可移除并清除进度
...
- 缓存上限 **0–500 MB**（默认 200；0 = 关闭流式预缓存；预缓存 **当前章 + 后两章**）
- 设置内可清除 **封面缓存** / **流媒体缓存**
